 ...
 */
export class SemanticSearchPlugin {
    constructor() {
        this.name = "semantic-search";
        this.description = "Enables semantic search using text embeddings to find semantically similar content";
        this.version = "1.0.0";
        this.config = {};
                ...
                switch (action) {
                    ...
                    case "info":
                        ...
                        const embInfo = `# Embeddings for "${embedding.title}"\n\n` +
                            ...
                            `## Sample Chunks\n\n` +
                            embedding.chunks.slice(0, 3).map((chunk, i) => `### Chunk ${i + 1}\n${chunk.substring(0, 150)}...`).join('\n\n');
                        return {
                            content: [{
                                    type: "text",
                                    text: embInfo
    ...
    chunkText(text, chunkSize, overlap) {
        ...
        while (currentIndex < text.length) {
            ...
            if (chunk.length < 10)
                break; // Stop if chunks are too small
            chunks.push(chunk);
    ...
    async generateEmbedding(text) {
        ...
        const cachedVector = this.embedCache.get(text);
        if (cachedVector) {
            return cachedVector;
        }
